...
class batch_opt_nonhol():
	...
	@partial(jit, static_argnums=(0,))	
	def compute_x(self, lamda_x, lamda_y, b_eq_x, b_eq_y, alpha_a, d_a, alpha_v, d_v, x_obs, y_obs, alpha_obs, d_obs, s_lane, y_lane_lb, y_lane_ub, v_des):
		

		A_vd = self.Pddot_jax-self.k_p_v*self.Pdot_jax
		b_vd = -self.k_p_v*jnp.ones((self.num_batch, self.num ))*(v_des)
	...
	def compute_projection(self, initial_state, lamda_x, lamda_y, x_obs, y_obs, x_guess, y_guess, c_x_bar, c_y_bar, y_lane_lb, y_lane_ub, alpha_a, d_a, alpha_v, d_v, s_lane, v_des):
		...
		for i in range(0, self.maxiter):
			...
			c_x, c_y, x, y, xdot, ydot, xddot, yddot, res_lane_vec, s_lane = self.compute_x(lamda_x, lamda_y, b_eq_x, b_eq_y, alpha_a, d_a, alpha_v, d_v, x_obs, y_obs, alpha_obs, d_obs, s_lane, y_lane_lb, y_lane_ub, v_des)
