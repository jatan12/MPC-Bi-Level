...
class batch_opt_nonhol():
	...
	def __init__(self, Wandb, BN, inp_mean, inp_std):
		...
		#################################################
		# self.maxiter = 20 # OG is 20
		# self.maxiter_cem = 5 # Ablation from 2 - 5 OG is 5

		#################################################
		self.maxiter = 20 # OG is 20
		self.maxiter_cem = 5 # Ablation from 1 - 5 OG is 5
	...
	def compute_x(self, lamda_x, lamda_y, b_eq_x, b_eq_y, alpha_a, d_a, alpha_v, d_v, x_obs, y_obs, alpha_obs, d_obs, c_x_bar, c_y_bar, s_lane, y_lane_lb, y_lane_ub, neural_output_batch):
		...
		cost_primary_x = self.cost_smoothness+self.rho_v*jnp.dot(A_vd_1.T, A_vd_1)+self.rho_v*jnp.dot(A_vd_2.T, A_vd_2)+self.rho_v*jnp.dot(A_vd_3.T, A_vd_3)+self.rho_v*jnp.dot(A_vd_4.T, A_vd_4)
		cost_primary_y = self.cost_smoothness+self.rho_offset*jnp.dot(A_pd_1.T, A_pd_1)+self.rho_offset*jnp.dot(A_pd_2.T, A_pd_2)+self.rho_offset*jnp.dot(A_pd_3.T, A_pd_3)+self.rho_offset*jnp.dot(A_pd_4.T, A_pd_4)
		
		lincost_primary_x = -self.rho_v*jnp.dot(A_vd_1.T, b_vd_1.T).T-self.rho_v*jnp.dot(A_vd_2.T, b_vd_2.T).T-self.rho_v*jnp.dot(A_vd_3.T, b_vd_3.T).T-self.rho_v*jnp.dot(A_vd_4.T, b_vd_4.T).T
		lincost_primary_y = -self.rho_offset*jnp.dot(A_pd_1.T, b_pd_1.T).T-self.rho_offset*jnp.dot(A_pd_2.T, b_pd_2.T).T-self.rho_offset*jnp.dot(A_pd_3.T, b_pd_3.T).T-self.rho_offset*jnp.dot(A_pd_4.T, b_pd_4.T).T 
		...
		# cost_x = cost_primary_x+self.rho_obs*jnp.dot(self.A_obs.T, self.A_obs)+self.rho_ineq*jnp.dot(self.A_acc.T, self.A_acc)+self.rho_ineq*jnp.dot(self.A_vel.T, self.A_vel)
		# cost_y = cost_primary_y+self.rho_obs*jnp.dot(self.A_obs.T, self.A_obs)+self.rho_ineq*jnp.dot(self.A_acc.T, self.A_acc)+self.rho_ineq*jnp.dot(self.A_vel.T, self.A_vel)+self.rho_lane*jnp.dot(self.A_lane.T, self.A_lane)

		# cost_mat_x = jnp.vstack((  jnp.hstack(( cost_x, self.A_eq_x.T )), jnp.hstack(( self.A_eq_x, jnp.zeros(( jnp.shape(self.A_eq_x)[0], jnp.shape(self.A_eq_x)[0] )) )) ))
		
		# cost_mat_y = jnp.vstack((  jnp.hstack(( cost_y, self.A_eq_y.T )), jnp.hstack(( self.A_eq_y, jnp.zeros(( jnp.shape(self.A_eq_y)[0], jnp.shape(self.A_eq_y)[0] )) )) ))
		
		# lincost_x = -lamda_x-lincost_primary_x-self.rho_obs*jnp.dot(self.A_obs.T, b_obs_x.T).T-self.rho_ineq*jnp.dot(self.A_acc.T, b_ax_ineq.T).T-self.rho_ineq*jnp.dot(self.A_vel.T, b_vx_ineq.T).T
		# lincost_y = -lamda_y-lincost_primary_y-self.rho_obs*jnp.dot(self.A_obs.T, b_obs_y.T).T-self.rho_ineq*jnp.dot(self.A_acc.T, b_ay_ineq.T).T-self.rho_ineq*jnp.dot(self.A_vel.T, b_vy_ineq.T).T-self.rho_lane*jnp.dot(self.A_lane.T, b_lane_aug.T).T
	...
	@partial(jit, static_argnums=(0, ))
	def compute_cost(self, y_ellite_projection, res_ellite_projection, xdot_ellite_projection, ydot_ellite_projection, xddot_ellite_projection, yddot_ellite_projection, y_lane_lb, y_lane_ub, v_des, steering_ellite_projection, d_v_ellite_projection, x_ellite_projection, velocity_scaling):
		...
		heading_angle_change = jnp.linalg.norm(jnp.diff(heading_angle, axis = 1), axis = 1)
		...
		# cost_batch = res_ellite_projection+0.00001*jnp.linalg.norm(xdot_ellite_projection-v_des, axis = 1)+0.01*cost_steering+0.01*cost_steering_vel+heading_penalty#+0.001*jnp.linalg.norm(y_ellite_projection-(y_lane_ub+y_lane_lb)/2, axis = 1  )
		
		cost_batch = res_ellite_projection+velocity_scaling*jnp.linalg.norm(xdot_ellite_projection-v_des, axis = 1)+0.01*cost_steering+0.01*cost_steering_vel+heading_penalty#+0.001*jnp.linalg.norm(y_ellite_projection-(y_lane_ub+y_lane_lb)/2, axis = 1  )
		
		# cost_batch = res_ellite_projection+jnp.linalg.norm(xdot_ellite_projection-v_des, axis = 1)#+0.01*cost_steering+0.01*cost_steering_vel+heading_penalty#+0.001*jnp.linalg.norm(y_ellite_projection-(y_lane_ub+y_lane_lb)/2, axis = 1  )
		
		
		# cost_batch = res_ellite_projection-x_ellite_projection[:, -1]#+0.01*cost_steering+0.01*cost_steering_vel+heading_penalty#+0.001*jnp.linalg.norm(y_ellite_projection-(y_lane_ub+y_lane_lb)/2, axis = 1  )
	...
	@partial(jit, static_argnums=(0, ))
	def compute_shifted_samples(self, key, neural_output_ellite, cost_batch, idx_ellite, mean_param_prev, cov_param_prev, i):

		
		temp = jnp.maximum(1, (16.0/((i+1))  ))
	...
	def compute_cem(self, initial_state, lamda_x, lamda_y, x_obs, y_obs, y_lane_lb, y_lane_ub,  alpha_a, d_a, alpha_v, d_v, v_des, s_lane, mean_param, cov_param, neural_output_warmstart):
		...
		# mean_param = jnp.mean(neural_output_batch, axis = 0)
		# cov_param = jnp.cov(neural_output_batch.T)

		dist_obs = jnp.sqrt((0-x_obs[:, 0])**2+(0-y_obs[:, 0])**2)

		dist_res = jnp.maximum(0,  jnp.min(dist_obs)-10  )

		velocity_scaling = 0.001#*(1+jnp.exp(dist_res))
		...
		for i in range(0, self.maxiter_cem):
			...
			mean_param, cov_param, neural_output_batch, cost_batch_temp = self.compute_shifted_samples(key, neural_output_ellite, cost_batch, idx_ellite, mean_param_prev, cov_param_prev, i)
		...
		return 	c_x_best, c_y_best, neural_output_ellite[idx_min], neural_output_ellite
	...
	def compute_cem_nn(self, inp, initial_state, lamda_x, lamda_y, x_obs, y_obs, y_lane_lb, y_lane_ub,  alpha_a, d_a, alpha_v, d_v, v_des, s_lane, mean_param, cov_param, neural_output_warmstart):
		...
		for i in range(0, self.maxiter_cem):
			...
			idx_min = jnp.argmin(cost_batch_temp)
			# res_2.append(cost_batch_temp[idx_min])
