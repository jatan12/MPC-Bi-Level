...
def get_params(four_lane=True):
    ...
    if four_lane:
        model.load_state_dict(torch.load('./weights/cvae_ddn_four_lane.pth')) 
    else:
        model.load_state_dict(torch.load('./weights/cvae_ddn_two_lane.pth'))
    model.eval()
